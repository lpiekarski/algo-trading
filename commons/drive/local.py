...
def upload(local_path: str, cloud_path: str) -> None:
	LOGGER.debug(f"Local drive uploading file from '{formpath(local_path)}' to '{formpath(cloud_path)}'")
...
def download(cloud_path: str, local_path: str) -> None:
	if not os.path.exists(cloud_path):
		raise CloudFileNotFoundError(f"Cannot find file in location: '{cloud_path}'")
	LOGGER.debug(f"Local drive copying file from '{formpath(cloud_path)}' to '{formpath(local_path)}'")
	if os.path.normpath(local_path) != os.path.normpath(cloud_path):
...
def delete(cloud_path: str) -> None:
	if not os.path.exists(cloud_path):
		raise CloudFileNotFoundError(f"Cannot find file in location: '{cloud_path}'")
	LOGGER.debug(f"Local drive deleting file from '{os.path.normpath(cloud_path)}'")
