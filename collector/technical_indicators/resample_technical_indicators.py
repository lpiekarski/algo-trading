...
def resample_technical_indicators(df, time_tag="1h"):
    ...
    for index, row in df.iterrows():
        ...
        result_df = pd.concat([result_df, pd.DataFrame([last_record])])
    return result_df
...
if __name__ == '__main__':
    ...
    resample_df = df.resample("1h").apply(ohlc)  # 5min #30min #1h #1d #1w #1m

    print(df)
    print(resample_df)
    print(resample_technical_indicators(df, time_tag="1h"))